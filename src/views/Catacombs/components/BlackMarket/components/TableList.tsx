...
const TableList: React.FC<TableListProps> = ({ id }) => {
  ...
  const renderButtons = () => {
    if (cancelled) {
      return (
        <button className="rug-market-listing-buttons" type="button" disabled>
          Cancelled
        </button>
      )
    }
    ...
    if (isOwner) {
      return (
        <button onClick={handleCancel} className="rug-market-listing-buttons" type="button">
          Cancel listing
        </button>
      )
    }
    ...
    if (isApproved) {
      return (
        <button onClick={handleBuy} className="rug-market-listing-buttons" type="button">
          Swap
        </button>
      )
    }
    ...
    return (
      <button onClick={handleApprove} className="rug-market-listing-buttons" type="button">
        Approve ZMBE
      </button>
    )
  }
      ...
      <tbody>
        <tr>
            ...
            <div className="info-td-one">
              <div className="into-two-td">
                <div className="info-1">
                  ...
                  <div>
                    <div className="titel">
                      Swap {getFullDisplayBalance(new BigNumber(listing.quantity), listing.token?.decimals, 2)}{' '}
                      {listing.token?.symbol} for {getFullDisplayBalance(new BigNumber(listing.price), 18, 1)} ZMBE
                    </div>
                  </div>
                </div>
              </div>
          ...
          <td className="barrack-td-width-10">
            <DisplayFlex>{renderButtons()}</DisplayFlex>
          </td>
        </tr>
      </tbody>
    </table>
