    ...
    <div>
      <Card className="card-active">
        ...
        <CardBody style={{ padding: '18px 30px' }}>
          <Flex justifyContent="center" style={{ color: 'white!important', lineHeight: 'normal', fontSize: '18px' }}>
        ...
        <CardFooter>
          ...
          <select onChange={selectRuggedToken} className="SelectRuggedToken">
            {ruggedTokens.map((rugSymbol) => {
              ...
              return <option value={rugSymbol}>{rug.symbol}</option>
            })}
          </select>
          {
          ...
          <Flex>
            {/* eslint-disable-next-line no-nested-ternary */}
            {rugBalance.lt(BIG_TEN.pow(selectedRug.decimals)) ? (
              <StyledButton variant="secondary">Insufficient {tokens[ruggedToken].symbol}</StyledButton>
            ) : !rugApproved ? (
            ...
            <StyledButton
              variant="secondary"
              disabled={
                !rugApproved || rugBalance.lt(BIG_TEN.pow(selectedRug.decimals)) || zombieBalance().lt(burnAmount)
              }
              onClick={rugRoll}
            >
