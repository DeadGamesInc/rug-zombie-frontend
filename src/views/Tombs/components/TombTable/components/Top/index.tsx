...
const Top: TableTop<Tomb> = ({ target: tomb, open }) => {
  ...
  const bigTvl = tokenAmount.times(lpPriceBnb).times(useGetBnbPriceUsd())
  const tvl = getBalanceNumber(bigTvl)
  const bigZombiePrice = getDecimalAmount(new BigNumber(useGetZombiePriceUsd()))
  ...
  return (
    <TombColumn>
      <TombHeaderRow>
        ...
        <TombTitle>{lpName}</TombTitle>
        <TabFlex>
          <GreenTab>
            <GreenTabText>{allocPoint.div(100).toString()}X</GreenTabText>
          </GreenTab>
          <GreyTab>
            <GreyTabText>{DEXS[dex]}</GreyTabText>
          </GreyTab>
          {isNew ? (
            <GreenTab>
